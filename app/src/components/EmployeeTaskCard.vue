...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            detail: function (id) {
                let items = this.$store.getters.EDFS;
                if (items !== null) {
                    for (let item of items) {
                        if (item['id'] === id) {
                            this.$store.commit('setEmployeeUpdate', {type: 'Item', val: true, data: item});
                            this.$router.push('/employee/requests/detail');
                            break;
                        }
                    }
                } else {
                    if(this.$router.currentRoute.name !== 'employee_home'){
