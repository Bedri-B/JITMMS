        ...
        <div class="min-w-fit flex flex-col w-3/4 mr-4">
            <div class="mb-4 flex justify-between items-start">
                <div class="flex items-end">
                    <h1 class="text-2xl leading-none text-black-1 mr-2">Role</h1>
                    <span class="text-sm leading-none text-block-3">{{ roleList.data.length }} Total</span>

                </div>
                <RouterLink to="/admin/role/manage">
                    <Button class="w-56 px-4 py-2 ml-10 bg-primary-normal text-white hover:bg-primary-light" rounded>
                        New Role
                    </Button>
                </RouterLink>
            </div>
            <RoleTable :actions="roleList.actions"
...
<script>
    ...
    import RoleTable from "../../../components/Admin/Table";
    ...
    export default {
        ...
        methods: {
            updateRoleInfo: function (id) {
                this.updateRole('update', id)
            },
            updateRole(type, id) {
                let role = null;
                for (let man of this.roleList['all']) {
                    if (man['id'] === id) {
                        role = man;
                        break;
                    }
                ...
                if (role === null) return;
                if (type === 'update') {
                    this.$store.commit('setAdminUpdate', {type: 'Role', val: true, data: role});
                    this.$router.push('/admin/role/manage');
                }
            },
