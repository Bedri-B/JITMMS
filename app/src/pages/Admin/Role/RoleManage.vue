...
<script>
    ...
    export default {
        ...
        components: {},
        created: function () {
            if (this.$store.getters.ARU) {
                this.title = "Update Role";
                this.action = "Update";
                this.update = true;
                let data = this.$store.getters.ARD;
                this.id = data["id"];
                this.name = data["name"];
                this.description = data["description"];

                this.$store.commit('setAdminUpdate', {type: 'Role', val: false, data: null});
            }
        },
        methods: {
            reset: function () {
        ...
        await axios(this.$store.getters.API + "/api/role/update", {
                        method: "post",
                        headers: {
                        ...
                        .then((response) => {
                            ...
                            if (res.status === 200) {
                                this.$toast.success("Role information updated successfully!");
                    ...
                    if(!this.$store.getters.AuthStatus) return;
        await axios(this.$store.getters.API + "/api/role/create", {
                        method: "post",
                        headers: {
                            "Content-Type": "multipart/form-data",
                            "x-access-token": this.$store.getters.AuthToken,
