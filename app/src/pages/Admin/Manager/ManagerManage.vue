<template>
    <main class="flex flex-col w-full p-10 max-h-full">
        ...
        <div class="mx-auto w-4/5 p-6 shadow-lg rounded-lg bg-primary-light text-gray-900">
            ...
            <div class="flex flex-row sm:flex-col" v-if="update">
...
<script>
    ...
    export default {
        ...
        components: {},
        created: function () {
            if (this.$store.getters.AMU) {
                this.title = "Update Manager";
                this.action = "Update";
                this.update = true;
                let data = this.$store.getters.AMD;
                this.id = data["id"];
                ...
                this.$store.commit('setAdminUpdate', {type: 'Manager', val: false, data: null});
            }
        },
        methods: {
            reset: function () {
            ...
            Create: async function () {
                ...
                if (this.update) {
                    ...
                    await axios(this.$store.getters.API + "/api/manager/update", {
                        method: "post",
                        headers: {
                        ...
                        .then((response) => {
                            ...
                            if (res.status === 200) {
                                this.$toast.success("Manager information updated successfully!");
                    ...
                    if (!this.$store.getters.AuthStatus) return;
                    await axios(this.$store.getters.API + "/api/manager/create", {
                        method: "post",
                        headers: {
                            "Content-Type": "multipart/form-data",
                            "x-access-token": this.$store.getters.AuthToken,
